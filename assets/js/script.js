...
e.preventDefault();
	...
	$('.btn-stepnext').click(function (e) {
		...
		if ('tb_shipping' == currentActive) {
			...
			var addreess = fname.val() + ' ' + lname.val() + ', <br> ' + adderss1.val() + ' <br/> ' + adderss2.val() + ' <br/>' + phone.val() + ', <br/>' + city.val() + ' ' + post.val() + ', ' + country.val();
			// $('.billing-address-wrp-plain').html(addreess);
			var valid = true;
			if (fname.val() == '' || lname.val() == '' || adderss1.val() == '' || country.val() == '' || city.val() == '' || post.val() == '' || phone.val() == '') {
				valid = false;
			}
			if (valid) {
			...
			var addreess = fname.val() + ' ' + lname.val() + ', <br> ' + adderss1.val() + ' <br/>' + adderss2.val() + ' , <br/>' + city.val() + ' ' + post.val() + ', ' + country.val();
			var valid = true;
			if (fname.val() == '' || lname.val() == '' || adderss1.val() == '' || country.val() == '' || city.val() == '' || post.val() == '' || phone.val() == '') {
				valid = false;
			}
