...
<main id="main-content">
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            //billing-address-wrp-plain
            var shippingCountry = '<div class="step-section-header mb-1"><h6 class="step-section-title">Shipping Country</h6></div>';
            $('#shipping_country_field > label').remove();
            $('#shipping_country_field').prepend(shippingCountry);

            var shippingSub = '<div class="step-section-header"><h6 class="step-section-title"> Shipping ADDRESS</h6></div>';
            $('#shipping_country_field').after(shippingSub);

            var fname = $('#billing_first_name').val();
            var lname = $('#billing_last_name').val();
            var adderss1 = $('#billing_address_1').val();
            var adderss2 = $('#billing_address_2').val();
            var country = $('#billing_country').val();
            var city = $('#billing_city').val();
            var post = $('#billing_postcode').val();

            var addreess = fname + ' ' + lname + ', <br> ' + adderss1 + ' , ' + adderss2 + ' , <br/>' + city + ' ' + post + ', ' + country;
