...
function ajax_return_request()
{
    ...
    $order_item_id = sanitize_text_field($_POST['item_id']);
    ...
    $title = $order->get_order_number() . ' - ' . $product . ' - ' . $qty;
    $my_post = array(
        'post_title'    => $title,
        'post_type'  => 'returns',
        'post_status'   => 'publish',
...
function set_custom_edit_returns_columns($columns)
{
    ...
    $columns['order_id'] = __('Order ID', 'rawbought');
    $columns['item_id'] = __('Item', 'rawbought');
    $columns['total'] = __('Total', 'rawbought');
    $columns['status'] = __('Status', 'rawbought');
    return $columns;
}
...
function janio_api_call($order_id)
{
    ...
    if (array_key_exists($orderShippingCountry, $serviceList)) {
        ...
        update_post_meta($order_id, 'order_tracking_id', $vars['tracking_nos'][0]);
    }
}
